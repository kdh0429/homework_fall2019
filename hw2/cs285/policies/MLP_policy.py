...
class MLPPolicy(BasePolicy):
    ...
    def build_action_sampling(self):
        if self.discrete:
            ...
            self.sample_ac = mean + tf.exp(logstd) * tf.random_normal(tf.shape(mean), 0, 1) # Normal distribution에서 sampling
...
class MLPPolicySL(MLPPolicy):
    ...
    def update(self, observations, actions):
        ...
        _, batch_loss = self.sess.run([self.train_op, self.loss], feed_dict={self.observations_pl: observations, self.acs_labels_na: actions})
